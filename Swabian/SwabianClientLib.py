...
class TimeTag():
    ...
    def startTimeTagStreamFor(self): 
        ...
        self.TimeTagger._pyroClaimOwnership() 
        self.ttagData = np.array([], dtype = np.uint64())         
        self.stream = self.TimeTagger.TimeTagStream(self.tt, self.max_buf_size, self.activeChannels)
        time.sleep(0.05)
        #self.stream.clear()
...
if (__name__ == '__main__'):
    ...
    TT = TimeTag(IP,"TimeTagger.yaml", "Alice")
    TT.initCounts()
    print(TT.getCounts())
